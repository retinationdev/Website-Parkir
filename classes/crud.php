...
class crud extends dbConfig
{
	...
	function __construct()
	{
		...
		function checkUser($email,$licenseNo,$password){
		//print_r($email);
		$sql = "select * from `user` where `email` = '".$email."' OR `licenseNo` = '".$licenseNo."' AND `password`='".$password."' ";
		//print_r($email);
	...
	// checking licenseNo usng ajax
	function checklicenseNoAjax($email){
		$sql = "select * from `user` where `licenseNo` ='".$licenseNo."' ";
		$result = $this->conn->query($sql);
		if ($result->num_rows>0) {
			return true;
		}else{
	...
	function register($firstName,$lastName,$email,$password,$licenseNo,$phone,$role){
	...
	// // die();
								...
								// 	else{
															// 		return true;
								// 	}
		$sql = "INSERT INTO `user` (`user_id`, `firstName`, `lastName`, `email`, `password`, `licenseNo`, `role_id`, `blackListPoint`, `emailConfirmed`, `verificationCode`) VALUES (NULL, '$firstName', '$lastName', '$email', '$password', '$licenseNo', '$role', '', '', '')";
		$result = $this->conn->query($sql);
		if ($result == false) {
			return false;
		}else{
	...
	// update user
	function updateUser($licenseNo,$email){
		$sql = "UPDATE `user` `licenseNo` = '".$licenseNo."'
				WHERE
				`email` = '".$email."' ";
		$result = $this->conn->query($sql);
		return $result;
	...
	function getUserBooking($userId){
		$sql = "SELECT * FROM `booking_info` WHERE `user_id` ='".$userId."' order by `bookingDate` desc ";
		$result = $this->conn->query($sql);
		return $result;
	}
